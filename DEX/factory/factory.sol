...
contract CitadelFactory is IFactory {
    address public owner;
    address public feePercentOwner;
    address public setStableOwner;
    address public feeTo;

    bytes32 public constant INIT_CODE_HASH = keccak256(abi.encodePacked(type(CitadelPair).creationCode));
     ...
     */
    modifier onlyOwner() {
        require(owner == msg.sender, "Factory: caller is not the owner");
        _;
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        bytes memory bytecode = type(CitadelPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        require(pair != address(0), "Factory: FAILED");
        CitadelPair(pair).initialize(token0, token1);
    ...
    function setOwner(address _owner) external onlyOwner {
        require(_owner != address(0), "Factory: zero address");
        emit OwnershipTransferred(owner, _owner);
        owner = _owner;
    }

    function setFeePercentOwner(address _feePercentOwner) external onlyOwner {
        require(_feePercentOwner != address(0), "Factory: zero address");
    ...
    function setSetStableOwner(address _setStableOwner) external {
        require(msg.sender == setStableOwner, "Factory: not setStableOwner");
        require(_setStableOwner != address(0), "Factory: zero address");
    ...
    function setReferrerFeeShare(address referrer, uint referrerFeeShare) external onlyOwner {
        require(referrer != address(0), "Factory: zero address");
        require(referrerFeeShare <= REFERER_FEE_SHARE_MAX, "Factory: referrerFeeShare mustn't exceed maximum");
