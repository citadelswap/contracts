...
contract CitadelPair is IPair, UniswapV2ERC20 {
  ...
  modifier lock() {
    require(unlocked == 1, 'CitadelPair: LOCKED');
    unlocked = 0;
    _;
    unlocked = 1;
  }
  ...
  function _safeTransfer(address token, address to, uint value) private {
    ...
    require(success && (data.length == 0 || abi.decode(data, (bool))), 'CitadelPair: TRANSFER_FAILED');
  }
  ...
  function initialize(address _token0, address _token1) external {
    require(msg.sender == factory && !initialized, 'CitadelPair: FORBIDDEN');
    // sufficient check
    token0 = _token0;
    token1 = _token1;
  ...
  */
  function setFeePercent(uint16 newToken0FeePercent, uint16 newToken1FeePercent) external lock {
    require(msg.sender == IFactory(factory).feePercentOwner(), "CitadelPair: only factory's feeAmountOwner");
    require(newToken0FeePercent <= MAX_FEE_PERCENT && newToken1FeePercent <= MAX_FEE_PERCENT, "CitadelPair: feePercent mustn't exceed the maximum");
    require(newToken0FeePercent > 0 && newToken1FeePercent > 0, "CitadelPair: feePercent mustn't exceed the minimum");
  ...
  function setStableSwap(bool stable, uint112 expectedReserve0, uint112 expectedReserve1) external lock {
    require(msg.sender == IFactory(factory).setStableOwner(), "CitadelPair: only factory's setStableOwner");
    require(!pairTypeImmutable, "CitadelPair: immutable");

    require(stable != stableSwap, "CitadelPair: no update");
    require(expectedReserve0 == reserve0 && expectedReserve1 == reserve1, "CitadelPair: failed");
  ...
  function setPairTypeImmutable() external lock {
    require(msg.sender == IFactory(factory).owner(), "CitadelPair: only factory's owner");
    require(!pairTypeImmutable, "CitadelPair: already immutable");
  ...
  function _update(uint balance0, uint balance1) private {
    require(balance0 <= uint112(- 1) && balance1 <= uint112(- 1), 'CitadelPair: OVERFLOW');
  ...
  function mint(address to) external lock returns (uint liquidity) {
    ...
    require(liquidity > 0, 'CitadelPair: INSUFFICIENT_LIQUIDITY_MINTED');
    _mint(to, liquidity);
  ...
  function burn(address to) external lock returns (uint amount0, uint amount1) {
    ...
    require(amount0 > 0 && amount1 > 0, 'CitadelPair: INSUFFICIENT_LIQUIDITY_BURNED');
  ...
  function _swap(TokensData memory tokensData, address to, bytes memory data, address referrer) internal lock {
    require(tokensData.amount0Out > 0 || tokensData.amount1Out > 0, 'CitadelPair: INSUFFICIENT_OUTPUT_AMOUNT');

    (uint112 _reserve0, uint112 _reserve1, uint16 _token0FeePercent, uint16 _token1FeePercent) = getReserves();
    require(tokensData.amount0Out < _reserve0 && tokensData.amount1Out < _reserve1, 'CitadelPair: INSUFFICIENT_LIQUIDITY');


    {
      require(to != tokensData.token0 && to != tokensData.token1, 'CitadelPair: INVALID_TO');
    ...
    require(amount0In > 0 || amount1In > 0, 'CitadelPair: INSUFFICIENT_INPUT_AMOUNT');
    ...
    {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
      ...
      require(_k(balance0Adjusted, balance1Adjusted) >= _k(uint(_reserve0), uint(_reserve1)), 'CitadelPair: K');
    }
  ...
  function sync() external lock {
    ...
    require(token0Balance != 0 && token1Balance != 0, "CitadelPair: liquidity ratio not initialized");
  ...
  */
  function drainWrongToken(address token, address to) external lock {
    require(msg.sender == IFactory(factory).owner(), "CitadelPair: only factory's owner");
    require(token != token0 && token != token1, "CitadelPair: invalid token");
